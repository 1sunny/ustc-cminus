...
void CFGAnalyse::loop_find(Function *func) {
  ...
  while (!loop_stack.empty()) {
    ...
    loop_stack.pop();
    // 如果当前loop的header已经被设置过所在的外层loop了,就把outer_loop设置为那个外层loop
    // 这里一定是最接近的那个外层loop,因为算法是从外层逐步向内层循环的
    ...
    color[find_loop_entry(loop)] = 3; // 移除loop header,重新查找loop
    DFN = 0;
    BB_DFN.clear();
    BB_LOW.clear();
