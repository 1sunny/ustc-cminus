...
void ConstantPropagation::run() {
  // 创建支配树分析 Pass 的实例
  std::unique_ptr<Dominators> dominators_ = std::make_unique<Dominators>(m_);
  // 建立支配树
  dominators_->run();
  // 清空一些集合
  deadBlock.clear();
  ...
  for (auto fun: module_->get_functions()) {
    ...
    // 获取逆后序
    const std::vector<BasicBlock *> &reversePostOrder = dominators_->get_reverse_post_order(fun);
...
ConstantInt *ConstantPropagation::resCalc(Instruction::OpID op, ConstantInt *value1, ConstantInt *value2) {
...
ConstantFP *ConstantPropagation::resCalc(Instruction::OpID op, ConstantFP *value1, ConstantFP *value2) {
...
ConstantInt *ConstantPropagation::resCalc(CmpOp cmpOp, ConstantInt *value1, ConstantInt *value2) {
  int const_value1 = value1->get_value();
  int const_value2 = value2->get_value();
  bool boolRes;
  switch (cmpOp) {
...
ConstantInt *ConstantPropagation::resCalc(CmpOp cmpOp, ConstantFP *value1, ConstantFP *value2) {
  float const_value1 = value1->get_value();
  float const_value2 = value2->get_value();
  bool boolRes;
  switch (cmpOp) {
...
ConstantInt *ConstantPropagation::cast_to_const_int(Value *value) {
...
ConstantFP *ConstantPropagation::cast_to_const_float(Value *value) {
...
void ConstantPropagation::ConstFold(BasicBlock *block) {
  ...
  for (auto inst: block->get_instructions()) {
    ...
    if (inst->is_int_binary()) {
      ...
      Value *value1 = inst->get_operand(1);
      if (cast_to_const_int(value0) && cast_to_const_int(value1)) {
        ConstantInt *res = resCalc(dynamic_cast<CmpInst *>(inst)->get_cmp_op(), cast_to_const_int(value0),
                                   cast_to_const_int(value1));
        inst->replace_all_use_with(res);
        deleteInst.insert(inst);
      }
      ...
      Value *value1 = inst->get_operand(1);
      if (cast_to_const_float(value0) && cast_to_const_float(value1)) {
        ConstantInt *res = resCalc(dynamic_cast<FCmpInst *>(inst)->get_cmp_op(), cast_to_const_float(value0),
                                   cast_to_const_float(value1));
        inst->replace_all_use_with(res);
        deleteInst.insert(inst);
      }
  ...
  for (auto inst: deleteInst) {
    // 删除常量传播后的指令
    block->erase_instr(inst);
  }
}


void ConstantPropagation::DeadBlockEli(Function *fun) {
  for (auto block: fun->get_basic_blocks()) {
    // 删除phi指令中涉及死块的操作数
    for (auto inst: block->get_instructions()) {
      if (!inst->is_phi())break;
      bool test = false;
      for (int i = 0; i < (int) inst->get_num_operand() / 2; i++) {
...
void ConstantPropagation::BlockWalk(Function *fun, BasicBlock *block) {
  // 获取不可达基本块集合
...
void ConstantPropagation::RemoveSinglePhi(Function *fun) {
  for (auto block: fun->get_basic_blocks()) {
    std::set<Instruction *> deleteInst;
    for (auto inst: block->get_instructions()) {
      if (!inst->is_phi())break;
...
void ConstantPropagation::collect_unchanged_global_val() {
  //初始化unchangedGlobalVal
  auto globalVars = m_->get_global_variable();
