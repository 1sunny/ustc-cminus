#include "LocalCommonSubExpression.hpp"
#include "Dominators.hpp"

void LocalCommonSubExpression::run() {
  auto dom_tree = Dominators(module_);
  dom_tree.run();
...
void LocalCommonSubExpression::get_eliminatable_call() {
  for (auto func: module_->get_functions()) {
    auto can_eli = true;
...
bool LocalCommonSubExpression::is_call_eliminatable(std::string func_id) {
...
Arglist *LocalCommonSubExpression::get_call_args(CallInst *instr) {
...
void LocalCommonSubExpression::delete_expr_local(BasicBlock *bb) {
  //get pre_info
  if (bb != func_->get_entry_block()) {
    // 需要到达这一点的每条路径都计算了这个值,因此只能用支配这个基本块的对应信息
    ...
    // sotre,load不是单一定义,所以不能从直接支配者传递下来
    //    bb1
    // bb2   bb3
    //    bb4
    // 就像bb1的不能传递给bb4,因为bb2,bb3可能会对数组进行修改,但单一定义的变量就不会有这个情况
  ...
  for (auto instr: bb->get_instructions()) {
    if (instr->is_int_binary()) {
      ...
      if (const_offset) {
        if (load_off_val_const[bb].find({ptr, const_offset->get_value(), instr->get_instr_type()}) !=
            load_off_val_const[bb].end()) {
