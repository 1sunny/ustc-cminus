...
void FunctionInline::inline_call_find() {
...
void FunctionInline::func_inline() {
  for (auto pair: calling_pair) {
    ...
    // 把cur_bb中call指令后面的指令移动到split_bb
    ...
    for (auto succ_bb: cur_bb->get_succ_basic_blocks()) {
      ...
      for (auto succ_instr: succ_bb->get_instructions()) {
        if (succ_instr->is_phi()) {
          for (auto i = 0; i < succ_instr->get_num_operand(); i += 2) {
    ...
    auto entry_terminator = func_entry->try_get_terminator();
    ...
    // 把调用函数的每个基本块复制为new_bb,并复制里面的指令,phi指令直接添加到new_bb
      ...
      for (auto instr: bb->get_instructions()) {
        if (instr->is_phi()) {
          for (int i = 0; i < instr->get_num_operand(); i += 2) {
            // 操作数
            if (old2new_instr[instr->get_operand(i)] != nullptr) { // 全局变量不会被映射
    ...
    if (split_phi_pair.size() > 1) {
      // callee有多个ret语句的情况需要在split_bb创建phi
      ...
      // 用callee的ret_val替代原来接收返回值的变量
      // int x = f(); 用f里面的old2new_instr[ret_val]替代后续x的使用
